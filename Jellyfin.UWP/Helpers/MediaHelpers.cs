...
internal sealed class MediaHelpers(IMemoryCache memoryCache, JellyfinApiClient apiClient) : IMediaHelpers
{
    public async Task<Guid> GetPlayIdAsync(UIMediaListItem mediaItem)
    {
        ...
        return await GetPlayIdAsync(
            ...
            [.. seasons.Items.Select(x => new UIMediaListItem { Id = x.Id ?? Guid.Empty, Name = x.Name, Type = x.Type.Value, })],
            nextUpItem?.Id);
    }

    public Task<Guid> GetPlayIdAsync(BaseItemDto mediaItem, UIMediaListItem[] seriesData, Guid? seriesNextUpId = null)
    {
