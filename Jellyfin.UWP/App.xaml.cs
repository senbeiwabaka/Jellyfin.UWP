...
{
    private readonly MetroLog.ILogger Log;
...
#if DEBUG
        var minLogLevel = MetroLog.LogLevel.Debug;
#else
        var minLogLevel = MetroLog.LogLevel.Info;
#endif

        config.AddTarget(
            minLogLevel,
            MetroLog.LogLevel.Fatal,
        ...
        using (var scope = Ioc.Default.CreateScope())
        {
            ...
            if (!string.IsNullOrWhiteSpace(jellyfinUrl) || Uri.IsWellFormedUriString(jellyfinUrl, UriKind.Absolute))
            {
                ...
                try
                {
                    ...
                    logger.Log(Microsoft.Extensions.Logging.LogLevel.Debug, new Microsoft.Extensions.Logging.EventId(), "Test", null, (state, exception) => state);
                }
                catch (Exception ex)
                {
