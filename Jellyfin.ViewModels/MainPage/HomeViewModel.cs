...
public sealed class HomeViewModel(IMemoryCache memoryCache, JellyfinApiClient apiClient, IMediaHelpers mediaHelpers) : IHomeViewModel
{
        ...
        foreach (var record in mediaList.Where(x => x.CollectionType != BaseItemDto_CollectionType.Boxsets))
        {
            ...
            var itemsResult = await apiClient.Items.Latest
                .GetAsync(options =>
                {
                    ...
                    options.QueryParameters.Fields = [ItemFields.PrimaryImageAspectRatio, ItemFields.Path,];
